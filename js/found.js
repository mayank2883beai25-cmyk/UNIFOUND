...
(function () {
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      const dateFound = allInputs[4]?.value || "";
      ...
      if (!location) {
        showToast("Please enter the found location", "error");
        return;
      }

      if (!dateFound) {
        showToast("Please enter the date found", "error");
        return;
      }
      ...
      if (foundDate > today) {
        showToast("Date found cannot be in the future", "error");
        return;
      }
  ...
  function showSuccessModal(id) {
    ...
    modal.innerHTML = `
      <div class="uf-modal-card">
        ...
        <h2 class="uf-modal-title">Found Item Reported!</h2>
        <p class="uf-modal-subtitle">
          Thank you for reporting! Your submission has been saved and is pending review.
        </p>
        ...
        <div class="uf-modal-status">
          <span class="uf-modal-status-badge pending">Pending Verification</span>
        </div>
        <p class="uf-modal-message">
          Campus administrators will review this report and match it with any lost items.
        </p>
